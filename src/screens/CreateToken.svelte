<script lang="ts">
  ...
  export let fallback: ((json: any) => void) | undefined = undefined;

  async function create() {
    push(FullScreenLoader);
    const result = await connection.post("token", {}, { id: id });
    pop();
    ...
    if (result.ok) {
      ...
      if (fallback) {
        ...
        fallback({ [json["id"]]: json["token"] });
      }
    } else alert($_("failedToCreateTheToken"));
  }
</script>
...
<Stackable {zIndex}>
  ...
  <div class="container" transition:scale>
    <h3>{$_("createToken")}</h3>
    <Input bind:value={id} leftIcon={mdiIdentifier} placeholder={"ID"} />
    <div class="actions">
      <Button fullWidth warning outline on:click={pop}>{$_("close")}</Button>
      <Button fullWidth on:click={create}>{$_("create")}</Button>
    </div>
  </div>
</Stackable>
